...
curl -X POST -H 'Content-Type: application/json' --cert $(puppet config print hostcert) --key $(puppet config print hostprivkey) --cacert $(puppet config print localcacert) -d '{ "rule": ["or", ["=", "name", "sr8d6s08ofq6jql.delivery.puppetlabs.net"]] }' https://$(hostname -f):4433/classifier-api/v1/groups/9492547d-dbfb-4165-9b1c-f45fd143ed25
